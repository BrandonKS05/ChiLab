...
5. Open any Overleaf project, or click the ChiLab extension icon directly to use the popup Demo Mode.

## Deterministic Demo (no secrets required)

### Dashboard Demo Mode

1. Click the ChiLab extension icon in Chrome.
...
2. Look for the floating **ChiLab Review Mode** panel in the upper-right corner.
...
Expected output:
- ChiLab Journal Readiness score.
...
Run ChiLab as a deterministic CLI check without the extension:

```bash
npm run chilab:check -- --dir <path-to-latex-project>
```

### Output

The CLI writes two files:
- **`chilab-report.json`** — structured problem list (notation, undefined symbols, theorem issues).
...
```bash
npm run chilab:check -- --dir <path> --strict
```
...
To test a remote backend, set the `CHILAB_BACKEND_URL` environment variable:

```bash
CHILAB_BACKEND_URL=http://example.com:8000 npm run chilab:smoke:live
```
...
3. The report is Markdown titled **ChiLab Scientific Pre-Check Report**.

### Export Ledger (Extension)

1. In the extension popup, click **Export Ledger**.
2. A file `chilab-ledger.json` downloads with all review events.

### CLI Reports

After running `npm run chilab:check`, open:
- `chilab-report.md` — human-readable problems and fixes.
- `chilab-report.json` — structured data for tooling.
...
| `CHILAB_BACKEND_URL` | smoke test | Override backend URL | Default: http://localhost:8000 |
...
- CLI: `npm run chilab:check -- --dir <path>`.
...
- Smoke test (`npm run chilab:smoke:live`) against a running backend.
...
- **No multiplayer support**: ChiLab is a solo author pre-check tool. Concurrent document edits are not synchronized.
...
1. Open Chrome with the ChiLab extension loaded.
2. Open the ChiLab popup (click extension icon).
3. Ensure the **Readiness** tab is selected.

### Live in-document demo (3 min)

1. Open the extension popup and click the **Readiness** tab. Say: “ChiLab is running in Demo Mode because we don't have secrets configured. The dashboard scans LaTeX for notation problems.”
...
3. Click **Use Demo Paper** if it's not already active. Point to the problem symbol chips (`\sigma` and `\tau`) and say: “ChiLab found two issues: notation drift and use-before-definition.”
4. Click **Next Issue** to show the `\sigma` notation drift card. Read the “Why this matters” text aloud: “ChiLab explains why this matters to a reviewer. A human spellchecker wouldn't catch this.”
...
1. In a terminal, run: `npm run chilab:check -- --dir apps/overleaf-extension`
2. Open `chilab-report.md` and say: “The CLI produces the same pre-check without the UI. Useful for CI gates.”
3. Mention: `npm run chilab:check -- --dir <path> --strict` fails CI if issues found.

### Closing (1 min)

“ChiLab is a scientific publishing pre-check tool. It catches notation and theorem issues a grammar tool would miss. The deterministic path works today without secrets; the backend and Modal path unlock live model inference for stronger reasoning. The prototype is careful: it's labeled Demo Mode, not full certification.”
